...
- R7. Общий spend и круговая диаграмма включают только сопоставимые official cost series; balance, user budget и недоступные данные не суммируются со spend.
...
- `ProviderSnapshot`: capabilities, metrics, fetchedAt, source freshness, and optional typed error while retaining last success.
...
- A five-minute interval is a product default, not a freshness promise: OpenAI has no documented reporting latency, Anthropic usually updates within five minutes, DeepSeek publishes no balance SLA, and Gemini Basic has no usage polling.
